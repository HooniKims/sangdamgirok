...
const DISALLOWED_PAST_TENSE_PATTERN = /(했다|었다|였다|했음|었음|였음|되었다|하였다|보였다|보였으며|주었다|갔다|왔다|냈다|했으며|었으며)/;
const LEADING_DATE_PATTERN = /^\s*\d{4}년\s*\d{1,2}월\s*\d{1,2}일[,.]?\s*/;
...
export const validateBehaviorDraft = (
    ...
    if (DISALLOWED_PAST_TENSE_PATTERN.test(normalized)) {
        violations.push("과거형 종결어미(~했다/~었다/~였다 등)가 포함됨. 반드시 명사형(~함/~임/~음/~됨)으로 작성할 것.");
    }

    if (LEADING_DATE_PATTERN.test(text)) {
        violations.push("본문 앞에 날짜가 포함됨. 날짜 없이 바로 본문을 시작할 것.");
    }
...
[수정 지시]
- 위반 목록을 모두 해소할 것.
- 모든 문장을 반드시 명사형 종결어미(함/임/음/됨 등 받침 ㅁ) + 마침표(.)로 끝낼 것. "~했다/~었다/~였다" 등 과거형 종결어미는 절대 금지.
- 줄바꿈, 번호, 제목, 따옴표, 메타 설명 없이 한 문단 본문만 출력할 것.
- 날짜(YYYY년 M월 D일 등)를 본문에 절대 포함하지 않을 것.
- 본문 첫 글자부터 바로 내용을 시작할 것.
`.trim();
...
[절대 금지 - 반드시 지켜야 한다]
★ 날짜를 절대 쓰지 않는다. "2026년 2월 28일" 같은 날짜 표현을 본문에 포함하면 실패다.
★ 과거형 종결어미를 절대 쓰지 않는다. "~했다", "~었다", "~였다", "~보였다", "~주었다" 등은 금지다.
★ 모든 문장은 반드시 명사형 종결어미(~함, ~임, ~음, ~됨)로 끝낸다. 예외 없다.

[올바른 종결어미 예시]
✓ "적극적인 자세를 보임." ✓ "노력과 용기를 보여줌." ✓ "성장이 확인됨." ✓ "실천함."
[잘못된 종결어미 예시 - 절대 사용 금지]
✗ "자세를 보였다." ✗ "용기를 보여주었다." ✗ "확인되었다." ✗ "실천했다."
...
8) 글자수 지침을 반드시 준수한다.
9) 메타 설명, 분석, 검증, 글자수 표기 없이 본문 텍스트만 출력한다.
10) 날짜(YYYY년 M월 D일 등)를 본문에 절대 포함하지 않는다.
...
13) 최종 출력 직전 자체 점검을 수행한다.
    - 문장별 종결어미가 모두 받침 ㅁ(~함/~임/~음/~됨)인지 확인함. "~했다/~었다/~였다"가 있으면 무조건 재작성함.
    - 본문에 날짜가 있으면 무조건 제거 후 재작성함.
...
출력 형식
1) 오직 행발 본문 텍스트만 출력함. 날짜를 절대 포함하지 않음.
2) 줄바꿈 없이 하나의 문단으로 출력함.
3) 따옴표, 번호, 제목, 메타 설명 없이 본문만 출력함.
4) 모든 문장을 반드시 명사형 종결어미(~함/~임/~음/~됨, 받침 ㅁ) + 마침표(.)로 끝낼 것. "~했다/~었다/~였다" 등 과거형은 절대 금지.
5) 본문 첫 글자부터 바로 내용을 시작함. 앞에 날짜나 접두어를 붙이지 않음.
`.trim();
};
